...
class BinarySearchTree:
    ...
    def _find_path(self, element_1, element_2, nature):
        ...
        self.list_1 = []
        self.list_2 = []
        while element_1 is not None:
            key1 = element_1.key
            self.list_1.append(key1)
            element_1 = element_1.parent
        while element_2 is not None:
            key2 = element_2.key
            self.list_2.append(key2)
        ...
        for i in range(0, len(self.list_1)):
            for j in range(0, len(self.list_2)):
                if self.list_1[i] == self.list_2[j]:
                    pos1 = i
                    pos2 = j
                    break
        if nature == 0:
            if pos1 is not -1:
                list_path = self.list_1[0:pos1+1]
                self.list_2 = self.list_2[0:pos2]
                k = len(self.list_2)-1
                while k >= 0:
                    list_path.append(self.list_2[k])
                    k -= 1
        else:
            if pos2 is not -1:
                list_path = self.list_2[0:pos2+1]
                self.list_1 = self.list_1[0: pos1]
                k = len(self.list_1) - 1
                while k >= 0:
                    list_path.append(self.list_1[k])
                    k -= 1
